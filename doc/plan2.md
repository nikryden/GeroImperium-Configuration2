...
| 12.6 | App rewrite: `SyncViewModel` diff-and-push over `GeroImperiumClient` directly (no IPC for CRUD); Applications page adds page/order editing (`ApplicationPages` UI, currently has none); Key editor's shortcut UI compiles through `ChordSyntax`; connectivity indicator on REST-poll + BLE-presence. | PC Application Lead | Data & Sync Protocol Owner | Partially done, pulled into 12.5 out of necessity -- `SyncViewModel`'s diff-and-push core is done (see 12.5's row). Still not done: `ApplicationPages` page/order editing UI (Applications page still has none), REST-poll + BLE-presence connectivity indicator (today's indicator is only "REST connected or not," set once at Connect time), and "skip if unchanged"/delete-tombstone handling (still open decisions). **Not yet run against the real device** -- local `RemoteId`s are all null (never synced from this codebase before), and the real hardware already has substantial independently-configured data (12.3's verification found 364 live `keyactions` rows), so a first real Sync click would create all-new rows via POST rather than adopt/match the existing ones (the "full device -> local pull" open decision, still unresolved). Do not run Sync against that device without a deliberate plan for that collision. |

...
NuGet: `WPF-UI` (package id is `WPF-UI`, not `Wpf.Ui` -- that id resolves to an unrelated .NET-Framework-only package; App shell/theme/nav), `Microsoft.Data.Sqlite`, `SixLabors.ImageSharp` (png/jpeg; pinned to 3.1.11 -- v4 requires a commercial license key at build time), `Svg.Skia` + `SkiaSharp` (svg rasterization), `System.IO.Hashing` (Crc32 -- matches `esp_rom_crc32_le`/zlib per protocol doc), `System.IO.Ports` (SerialPort), `System.Management` (WMI device-change events, Windows-only), `CommunityToolkit.Mvvm`.

~~Open item: WPF-UI ships no ColorPicker.~~ Resolved in 11.4: hand-rolled `ColorSwatchPicker` (preset swatches + hex entry), see phase notes below.
...
* Add app-only columns: `GeroImperiumKeys.BackgroundColorArgb` (int), `Applications.BackgroundColorArgb`, `KeyActions.ActionType` (0=Shortcut, 1=LaunchApp, 2=Script -- see [Action types](#action-types-and-the-firmware-gap)), `KeyActions.LaunchPath` (text, app-only), `KeyActions.ScriptId` (FK, app-only). Also `Applications.SourceImageData`/`GeroImperiumKeys.SourceImageData` (added in 11.4): the pre-composite original upload, kept so a later background-color change can re-composite from scratch -- `ImageData` holds the already-flattened composited PNG preview, and re-running the compositor on that would be a no-op (no transparency left for a new background to show through).
...
| 11.4 | WPF shell + Applications/Key groups editor pages, dark/light theme. | PC Application Lead | — | Done — FluentWindow shell with WPF-UI NavigationView (content swapped via `ReplaceContent`, called from `Loaded` since the content presenter isn't ready at construction time -- caught by an actual app-launch smoke test, see below), dark theme via `ApplicationThemeManager`. Applications page (add/delete/rename/pick-image, live-persisted). Key Groups page (per-app group picker, 3x2 grid of the 6 fixed key slots, each with image/background/action-type/shortcut+modifiers/launch-path editing). Added `Core/Data/GeroImperiumRepository` (CRUD + cascading deletes + action dedup, matching the device's own dedup behavior for `T`) with 10 new tests. `ColorSwatchPicker` hand-rolled control resolves the open ColorPicker item. Verified via `dotnet run` smoke tests (headless environment, no interactive click-through possible) -- caught and fixed two real startup bugs (ReplaceContent NRE before template applied; Mica backdrop requiring ExtendsContentIntoTitleBar) and confirmed the fully-populated 6-slot DataTemplate renders against seeded data. 44/44 Core tests passing. |
| 11.5 | Sync page (progress bar + status text, bulk vs. incremental routing). | PC Application Lead | — | Done — `Core/Protocol/DeviceConnection` opens the CDC port directly (no Service/IPC yet, that's 11.6) and wraps it in `GeroImperiumProtocolClient`. `SyncViewModel`/`SyncPage`: Connect/Disconnect, full-database bulk push with a progress bar and per-stage status text ("Uploading chunk...", "Finalizing -- device verifying CRC and rebooting..."), and an incremental section (Application/KeyGroup/Key pickers) exercising all three incremental commands (`I`/`J`/`T`) against the already-composited bytes from local edits. `CanNavigateAway` disables the Applications/KeyGroups nav items while a bulk push is in flight, since a dropped mid-bulk connection has no recovery path. Verified via `dotnet run` smoke tests against both the default view and the Sync page (temporarily set as startup view, with seeded data) -- no crashes; real device hardware not available in this environment, so Connect/push against actual firmware is unverified. 44/44 Core tests passing (no new Core tests this phase -- DeviceConnection is hardware-only like DevicePortLocator; SyncViewModel is UI-layer and covered by the smoke tests instead). |
